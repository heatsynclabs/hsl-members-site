...
type SessionResponse = { data: { session: Session | null }, error: AuthError | null }
...
export const useAuth = () => {
  const {setLoading, setSessionResponse, setAuthStateChange} = useSessionStore();
  ...
  React.useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(setAuthStateChange);
